...
def pipeline(file_name, space, bow, dct, classes, class_names, words_to_get, processed_folder, dims, kfold_hpam_dict,
             hpam_dict,
    ...
    hyper_param = KFoldHyperParameter.RecHParam(None, classes, class_names, pipeline_hpam_dict, kfold_hpam_dict, "dir",
                                                model_type,
                                                ...
                                                hpam_params=[dct_unchanged, dct, bow, dir_min_freq, dir_max_freq,
                                                             ...
                                                             words_to_get, space, name_of_class, model_type, classes,
...
def direction_pipeline(dct_unchanged, dct, bow, dir_min_freq, dir_max_freq, file_name, processed_folder,
                       ...
                       mcm, dev_percent, ppmi, kfold_hpam_dict, data_type, rewrite_all, top_scoring_dir=None,
    ...
    if (data_type == "sentiment" and no_below > 5000) or (
            no_below >= 10000 and data_type != "movies") or no_below > 10000:
        stream_rankings = True

    if stream_rankings:  # and no_below > 5000) or no_below > 10000:
        rank = GetRankingsStreamed(dirs, space, new_word2id_dict, rank_save, file_name, processed_folder, no_below,
                                   no_above)
    else:
    ...
    if stream_rankings:  # and no_below > 5000) or no_below > 10000:
        ndcg = GetNDCGStreamed(rankings, ppmi, new_word2id_dict, dct_unchanged.token2id, ndcg_save, file_name,
                               processed_folder + "rank/ndcg/", no_below, no_above)
    else:
        ndcg = GetNDCG(rankings, ppmi, new_word2id_dict, dct_unchanged.token2id, ndcg_save, file_name,
                       processed_folder + "rank/ndcg/", no_below, no_above)
    ...
    for i in range(len(score_array)):
        dir_fn = file_name + "_" + sc_name_array[i] + "_" + str(top_scoring_dir) + "_" + str(no_below) + "_" + str(
            no_above)
        gtr_save = SaveLoad(rewrite=rewrite_all)
        if stream_rankings:
            gtr = GetTopScoringRanksStreamed(dir_fn, gtr_save, processed_folder + "rank/", score_array[i],
                                             top_scoring_dir, rankings, new_word2id_dict)
        else:
            gtr = GetTopScoringRanks(dir_fn, gtr_save, processed_folder + "rank/", score_array[i], top_scoring_dir,
                                     rankings, new_word2id_dict)
...
def main(data_type, raw_folder, processed_folder, proj_folder="", grams=0, dir_min_freq=0.001,
    ...
    for ci in range(len(name_of_class)):
        ...
        for i in range(len(dims)):
            ...
            for s in range(len(spaces)):
                ...
                if data_type == "movies" or data_type == "placetypes":
                    ...
                    tsrd = pipeline(final_fn, spaces[s], bow, dct, classes, class_names, word_list, processed_folder,
                                    ...
                                    pipeline_hpam_dict=pipeline_hpam_dict, space_name=space_names[s],
                                    data_type=data_type)
                else:
        ...
        col_names = all_r[0][0]
        key = all_r[2]
        dt.write_csv(processed_folder + "rank/score/csv_final/" + final_fn + "reps" + model_type + "_" + name_of_class[
            ci] + ".csv", col_names, cols, key)
        print("a")


if __name__ == '__main__':
    data_types = ["placetypes", "reuters", "sentiment", "newsgroups"]
    doLR = False
    dminf = -1
    dmanf = -1
    ...
    for j in range(len(data_types)):
        ...
        main(data_types[j], "../../data/raw/" + data_types[j] + "/", "../../data/processed/" + data_types[j] + "/",
             proj_folder="../../data/proj/" + data_types[j] + "/",
             grams=0, dir_min_freq=dminf, dir_max_freq=dmanf, dev_percent=0.2,
             score_metric="avg_f1",multiclass=mcm, LR=doLR, bonus_fn=bonus_fn,
