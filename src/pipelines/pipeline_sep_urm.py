...
def main(data_type, raw_folder, processed_folder, proj_folder="", grams=0, model_type="LinearSVMNOvR", no_below=0.001,
    ...
    if data_type == "animecf":
        URM = sp.load_npz('E:/business/baseline-recommendation\Data_manager_split_datasets\Anime\original/URM_all.npz')
        import json
        # read file
        with open('E:/business/baseline-recommendation\Data_manager_split_datasets\Anime\original/item_original_ID_to_index.json', 'r') as myfile:
            data = myfile.read()
        # parse file
        words = np.load("E:/business/baseline-recommendation\Data_manager_split_datasets\Anime\myanimelist/genre_names.npy")
        index = json.loads(data)
        if True:
            tags = np.load("E:/business/baseline-recommendation\Data_manager_split_datasets\Anime\myanimelist/genre_matrix.npy")
            tags_index = dt.load_dict("E:/business/baseline-recommendation\Data_manager_split_datasets\Anime\myanimelist/genre_ids.npy")
            reverse_dict_URM = {}

            reverse_dict_tags = {}


            for key, items in tags_index.items():
                reverse_dict_tags[items] = key

            for key, items in index.items():
                reverse_dict_URM[items] = key

            index_to_index = {}

            for key, items in reverse_dict_URM.items():
                index_to_index[items] = reverse_dict_tags[key]

            new_tags = [None] * len(tags)
            for i in range(len(tags)):
                new_tags[
                    index[index_to_index[reverse_dict_tags[i]]]
                ] = np.asarray(tags[i], dtype="int8")
            new_tags = np.asarray(new_tags)
            tags = new_tags
            np.save("E:/business/baseline-recommendation\Data_manager_split_datasets\Anime\myanimelist/genre_matrix_indexed.npy", tags)
        else:
            tags = np.load("E:/business/baseline-recommendation\Data_manager_split_datasets\Anime\myanimelist/genre_matrix_indexed.npy")
        tags = tags.transpose()
        dct = None
