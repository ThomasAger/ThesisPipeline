...
def check_y(self):
    if len(self.true_targets) != len(self.predictions):
        raise ValueError("True targets do not equal length of self.predictions, True targets:", len(self.true_targets),
                         "self.predictions", len(self.predictions))
    if (pu.isList(self.predictions[0]) and not pu.isList(self.true_targets[0])) or (
        not pu.isList(self.predictions[0]) and pu.isList(self.true_targets[0])):
        raise ValueError("One of the targets/self.predictions are a 2d array, when both of them are expected to be.")
    if pu.isList(self.predictions[0]):
        if len(self.predictions[0]) != len(self.true_targets[0]):
            raise ValueError("Lengths of internal arrays do not match, self.true_targets", len(self.true_targets[0]),
