...
This Chapter begins by explaining the general process required to solving complex tasks with machine-learning starting from raw text data. The steps of this process are expanded on in the later sections.


Similar to how it would not be possible for a human to solve a problem without a good understanding of the subject area, the first step of solving a problem with a machine-learning model is to obtain a data representation that will be effective for the task. In this thesis raw text data must be converted into a form that is effective for the task of document classification. Document classification is the task of distinguishing between entities in a domain, where entities are e.g. movies in a domain of movie reviews, people in a domain of twitter posts, or reviews in a domain of product reviews. 

After obtaining the data from the domain, the first step of solving this task is usually separating the text into a document for each entity. Then, the text is pre-processed so that 
...
This thesis attempts to solve specific problems with text data using interpretable representations and classifiers. In this section, the basics of what the text data is, terminology associated with it and how it is preprocessed is described.
...
Bag-of-words representations are sparse, meaning that they take-up a lot of memory and need specialized machine-learning algorithms and data-structures to cater to them so that they are effective \cite{Sebastiani}. Dimensionality reduction is about reducing the number of dimensions while retaining the information. This is typically achieved by encoding the data in a bag-of-words in a low-dimensional vector space, where the location of the documents in that space is determined by their relative similarity to the other documents, so documents with similar ppmi vectors are closer together. 
