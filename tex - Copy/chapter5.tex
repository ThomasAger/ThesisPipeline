...
\hmark{This Chapter addresses two research questions:}
\begin{itemize}
...
\subsection{Quantitative Evaluation Of Feedforward Networks}
...
\hmark{This section evaluates if it is possible to obtain good  properties from the hidden layers of stacked denoising auto-encoders, following the same procedure as in Chapter \ref{ch4}. A representation composed of cluster properties is obtained from the hidden layers of the stacked denoising auto-encoder as described in Section \ref{howto}, then these representations are used as input to low-depth decision trees to classify the genres, keywords and ratings tasks for the movies domain. If the decision trees score higher or equal to the original input representation, we know that the properties represent key domain concepts as well as or better than the original representation. Otherwise we can expect that the representations do not contain properties that describe key domain concepts. The results are shown in Table \ref{quanautoresult}.}
...
% The properties are not good, really bad accuracy values

\hmark{The results of this experiment are negative. The accuracy and f1-scores of the properties obtained from the hidden layers of stacked auto-encoders is significantly lower than the properties obtained from the original representation. This signifies that the hidden-layers of these auto-encoders do not represent key domain properties. However, when observing examples these properties do seem to have meaning. This is further investigated in  Section \ref{auto-encoder-qual}.}
...
\subsection{Quantitative Frequency Evaluation}\ref{freq-eval}

\hmark{Although there is an indication that the properties in the later layers of a stacked denoising auto-encoder have some meaningful change, it is unclear what that change is. Following the qualitative analysis in Section \ref{auto-encoder-qual} it seems that the later properties are relevant to more documents, while the earlier properties are relevant to fewer documents. In other words, the earlier representations model properties that are more specific, in the sense that they are relevant to only some documents, and the later representations model properties that are more general, in the sense that they are relevant to more documents.} 

\hmark{Essentially, this section must investigate how frequent the clusters are.  The highest kappa score terms are added to the cluster first, which means that the initial words in the cluster are better modelled in the representation than those later others. Following this, we can understand that a more accurate representation of if more frequent terms are well modelled in the space can be obtained by taking the frequency of the first $N$ terms from each cluster and averaging them. Additionally, as there is a maximum cluster size, and all terms must be added, taking the top $N$ terms also removes the problem of terms being included that were added to clusters just because they were the least dissimilar compared to the other clusters, despite not being relevant to the cluster. 
	
\begin{table}[]
	\caption{Table!}\label{quan-freq-table}
\end{table}[]

In Table \ref{quan-freq-table} the average frequency of the clusters in each layer is shown for a variety of thresholds. Following the qualitative observations in Section \ref{auto-encoder-qual}, it is clear that as the auto-encoder layers get deeper, the frequency of the terms that are well modelled goes up. In conclusion, the stacked denoising auto-encoder models increasingly frequent terms.
