...
All tasks are evaluated as binary classification tasks. We randomly split the datasets into 2/3 for training and 1/3 for testing. \hmark{As in all other experiments, 20\% of the training data is taken for hyper-parameter tuning. As we are not tuning as many parameters and Place-Types has a low number of documents for its tasks, we use 5-fold cross validation in this domain for these experiments. }
